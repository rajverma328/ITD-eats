import os
from datetime import datetime, date, time as dtime
import threading

import pytz
from flask import Flask, render_template, request, redirect, url_for, session, jsonify
from flask_sqlalchemy import SQLAlchemy
from werkzeug.security import generate_password_hash, check_password_hash

# -----------------------
# Config
# -----------------------
SECRET_KEY = os.environ.get("SECRET_KEY", "change_this_secret_in_prod")
DATABASE_URL = os.environ.get("DATABASE_URL", "sqlite:///items.db")
# Preferred: set ADMIN_PASS_HASH env var to a werkzeug-generated hash of "floor3"
ADMIN_PASS_HASH = os.environ.get("ADMIN_PASS_HASH")
if not ADMIN_PASS_HASH:
    # fallback for quick testing only (do not leave this in a public repo)
    ADMIN_PASS_HASH = "scrypt:32768:8:1$INnuLziqpYVfTZ2d$5ae101b115844cca1f313cacd2323f10a81cc14127ad55b66a1e4097e8da5dc3ad342e37791713013f8e9333d241a2b5637f208c33624d8679cb3c27970d557a"

# timezone & reset time (3 PM)
RESET_TZ = os.environ.get("RESET_TZ", "Asia/Kolkata")
RESET_TIME = dtime(18, 0, 0)  # 15:00 = 3 PM singapore time

# -----------------------
# App + DB setup
# -----------------------
app = Flask(__name__, static_folder="static", template_folder="templates")
app.config["SECRET_KEY"] = SECRET_KEY
app.config["SQLALCHEMY_DATABASE_URI"] = DATABASE_URL
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False

db = SQLAlchemy(app)

# -----------------------
# Models
# -----------------------
class Item(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(120), nullable=False)
    votes = db.Column(db.Integer, default=0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

with app.app_context():
    db.create_all()

# -----------------------
# Reset-once-per-day logic (no APScheduler)
# -----------------------
_reset_lock = threading.Lock()
_last_reset_date = None  # stores date object of last reset (in RESET_TZ)

def maybe_reset():
    """
    Deletes all Item records once per day **after** RESET_TIME in RESET_TZ timezone.
    This function is safe to call on every request. It will perform the deletion
    at most once per day (per process). For multi-instance deployments, prefer
    a centralized job (cron / scheduled job) or a shared DB-based marker.
    """
    global _last_reset_date
    try:
        tz = pytz.timezone(RESET_TZ)
    except Exception:
        tz = pytz.UTC

    now = datetime.now(tz)
    today = now.date()

    # Fast path: if last reset already recorded for today and it's today's date, nothing to do
    if _last_reset_date == today:
        return

    # Only try to reset if current time is >= RESET_TIME
    if now.time() < RESET_TIME:
        return

    # Acquire lock and re-check (avoid race conditions)
    with _reset_lock:
        if _last_reset_date == today:
            return  # another thread already reset
        # perform reset: delete all items (you can change this to set votes=0)
        with app.app_context():
            Item.query.delete()
            db.session.commit()
            _last_reset_date = today
            app.logger.info(f"Daily reset executed at {now.isoformat()} ({RESET_TZ})")

# -----------------------
# Authentication
# -----------------------
def require_login(fn):
    from functools import wraps
    @wraps(fn)
    def wrapper(*args, **kwargs):
        if not session.get("logged_in"):
            return redirect(url_for("login", next=request.path))
        return fn(*args, **kwargs)
    return wrapper

# -----------------------
# Routes
# -----------------------
@app.route("/login", methods=["GET", "POST"])
def login():
    if request.method == "POST":
        pw = request.form.get("password", "")
        if check_password_hash(ADMIN_PASS_HASH, pw):
            session["logged_in"] = True
            session.permanent = True
            return redirect(url_for("index"))
        else:
            return render_template("login.html", error="Incorrect password")
    return render_template("login.html", error=None)

@app.route("/logout")
def logout():
    session.clear()
    return redirect(url_for("login"))

@app.route("/")
@require_login
def index():
    # ensure daily reset happens if needed
    maybe_reset()
    return render_template("index.html")

@app.route("/api/items", methods=["GET"])
@require_login
def api_items():
    maybe_reset()
    items = Item.query.order_by(Item.votes.desc(), Item.created_at.asc()).all()
    return jsonify([{"id": it.id, "name": it.name, "votes": it.votes} for it in items])

@app.route("/api/items", methods=["POST"])
@require_login
def api_add_item():
    maybe_reset()
    data = request.get_json() or {}
    name = (data.get("name") or "").strip()
    if not name:
        return jsonify({"error": "Name required"}), 400
    it = Item(name=name, votes=1)
    db.session.add(it)
    db.session.commit()
    return jsonify({"ok": True, "id": it.id, "name": it.name, "votes": it.votes})

@app.route("/api/items/<int:item_id>/vote", methods=["POST"])
@require_login
def api_vote(item_id):
    maybe_reset()
    it = Item.query.get_or_404(item_id)
    it.votes += 1
    db.session.commit()
    return jsonify({"ok": True, "id": it.id, "votes": it.votes})

@app.route("/health")
def health():
    return "OK", 200

# -----------------------
# Run
# -----------------------
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))
    # log useful startup info
    app.logger.info(f"Starting app (DB={DATABASE_URL}) Reset at {RESET_TIME} in TZ={RESET_TZ}")
    app.run(host="0.0.0.0", port=port)
